...
class GaussianDiffusion(nn.Module):
    def __init__(self, denoise_fn, rrdb_net, bae_net, timesteps=1000, loss_type='l1'):
        super().__init__()
        self.denoise_fn = denoise_fn
        # condition net
        self.rrdb = rrdb_net
        self.bae_model = bae_net
    ...
    def forward(self, img_hr, img_lr, img_lr_up, diff_ages=None, patient_condition=None, age=None, t=None, *args, **kwargs):
        x = img_hr
        ...
        p_losses, x_tp1, noise_pred, x_t, x_t_gt, x_0 = self.p_losses(x, t, cond, img_lr_up, diff_ages=diff_ages, patient_condition=patient_condition, age=age, *args, **kwargs)
        ret = {'q': p_losses}
        ...
        #Add time prediction model loss
        self.bae_model.eval()
        #with torch.no_grad():
        time_pred, _ = self.bae_model(img_lr, x_t, diff_ages=diff_ages)
        ret['bae_loss'] = F.l1_loss(time_pred, diff_ages.unsqueeze(1))
        print(ret['bae_loss'], time_pred, diff_ages)

        return ret, (x_tp1, x_t_gt, x_t), t

    def p_losses(self, x_start, t, cond, img_lr_up, diff_ages, patient_condition, age, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_tp1_gt = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_t_gt = self.q_sample(x_start=x_start, t=t - 1, noise=noise)
        noise_pred = self.denoise_fn(x_tp1_gt, t, cond, img_lr_up, diff_ages=diff_ages, patient_condition=patient_condition, age=age)
        x_t_pred, x0_pred = self.p_sample(x_tp1_gt, t, cond, img_lr_up, diff_ages=diff_ages, patient_condition=patient_condition, age=age, noise_pred=noise_pred)
    ...
    @torch.no_grad()
    def p_sample(self, x, t, cond, img_lr_up, diff_ages, patient_condition, age, noise_pred=None, clip_denoised=True, repeat_noise=False):
        if noise_pred is None:
            noise_pred = self.denoise_fn(x, t, cond=cond, img_lr_up=img_lr_up, diff_ages=diff_ages, patient_condition=patient_condition, age=age)
        b, *_, device = *x.shape, x.device
    ...
    @torch.no_grad()
    def sample(self, img_lr, img_lr_up, shape, diff_ages=None, patient_condition=None, age=None, save_intermediate=False):
        ...
        for i in it:
            img, x_recon = self.p_sample(
                img, torch.full((b,), i, device=device, dtype=torch.long), cond, img_lr_up, diff_ages=diff_ages, patient_condition=patient_condition, age=age)

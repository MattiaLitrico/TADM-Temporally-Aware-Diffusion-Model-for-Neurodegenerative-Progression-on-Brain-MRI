...
class SRDiffTrainer(Trainer):
    def build_model(self):
        ...
        bae_model = BAEModel(rrdb)
        if hparams['bae_ckpt'] != '' and os.path.exists(hparams['bae_ckpt']):
            print("Loading BAE checkpoint from ", hparams['bae_ckpt'])
            load_ckpt(bae_model, hparams['bae_ckpt'])
    ...
    def sample_and_test(self, sample, diff_ages=None, patient_condition=None, age=None):
        ...
        if patient_condition is None:
            patient_condition = sample['patient_condition']
        if age is None:
            age = sample['age']
        
        img_sr, rrdb_out = self.model.sample(img_lr, img_lr_up, img_hr.shape, diff_ages=diff_ages, patient_condition=patient_condition, age=age)
    ...
    def training_step(self, batch):
        ...
        patient_condition = batch['patient_condition']
        age = batch['age']

        """img_hr = img_hr[img_hr.sum() > 0]
        img_lr = img_lr[img_lr.sum() > 0]
        img_lr_up = img_lr_up[img_lr_up.sum() > 0]"""

        losses, _, _ = self.model(img_hr, img_lr, img_lr_up, diff_ages=diff_ages, patient_condition=patient_condition, age=age)
        total_loss = sum(losses.values())
        return losses, total_loss, {'img_hr':img_hr, 'img_lr':img_lr, 'img_lr_up':img_lr_up}
